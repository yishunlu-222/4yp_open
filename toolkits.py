...
# def initialize_GPU(args):   no need to make progress for now
#     # Initialize GPUs
...
def get_voxceleb2_datalist(args, path):
    with open(path) as f:
        strings = f.readlines()
        audiolist = np.array([os.path.join(args.data_path,
                            string.split()[0]).replace("\\","/") for string in strings])
        #replace replace("\\","/") only for windows
