...
def train_model(model, device, db_gen, optimizer, epoch,train_num_iter,writer_train=None):
	model.train()
...
#
	...
	torch.save({'epoch': epoch + 1, 'state_dict': model.state_dict(), 'train_num_iter':train_num_iter,
		   ...
		   '/mnt/4TB/yishun_4yp/result/rawnetorigin/checkpoint_{}.pth'.format(epoch + 1))
	return train_num_iter
...
def evaluate_model(mode, model, db_gen, device, l_utt, save_dir, epoch, l_trial):
	...
	with torch.set_grad_enabled(False):
		...
		if mode =='val':
			writer_val = SummaryWriter('runs/network_rawnetorigin/traintotal_val')
			...
			writer_eer = SummaryWriter('runs/network_rawnetorigin/traintotal_eer')
...
if __name__ == '__main__':
	dir_yaml =  '/home/weidi/yishun/project/ssh/network2/train_RawNetssh.yaml'
	...
	#tensorboard
	# pdb.set_trace()
	# writer_val = SummaryWriter('runs/network_rawnetfullorigin/traintotal_val')

	# writer_eer = SummaryWriter('runs/network_rawnetfullorigin/traintotal_eer')
	writer_train = SummaryWriter('runs/network_rawnetfullorigin/traintotal_train')
	best_eer = 99.
	...
	for epoch in tqdm(range(parser['epoch'])):
		#train phase
		# startfull = time.time()
		# train_num_iter=train_model(model = model,
			# device = device,
			# db_gen = devset_gen,
			# optimizer = optimizer,
			# epoch = epoch,
			# writer_train = writer_train,
			# train_num_iter=train_num_iter)
		# endtrain = time.time()
