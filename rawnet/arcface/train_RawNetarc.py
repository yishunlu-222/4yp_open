...
def train_model(model, device, db_gen, optimizer, epoch,train_num_iter,writer_train=None):
	...
	torch.save({'epoch': epoch + 1, 'state_dict': model.state_dict(), 'train_num_iter':train_num_iter,
		   ...
		   '/mnt/4TB/yishun_4yp/result/rawnetarcface0.3/checkpoint_{}.pth'.format(epoch + 1))
...
def evaluate_model(mode, model, db_gen, device, l_utt, save_dir, epoch, l_trial):
	...
	with torch.set_grad_enabled(False):
		...
		if mode =='val':
			writer_val = SummaryWriter('runs/rawnetarcface0.3/traintotal_val')
			...
			writer_eer = SummaryWriter('runs/rawnetarcface0.3/traintotal_eer')
...
if __name__ == '__main__':
	...
	metric_fc = metrics.ArcMarginProduct(512, parser['model']['nb_classes'], s=64, m=0.3)
	...
	##########################################
	#tensorboard
	writer_train = SummaryWriter('runs/rawnetarcface0.3/traintotal_train')
	best_eer = 99.
